...
- (void)setDataList:(NSArray *)dataList
{
    ...
    for (int i = 0; i < self.dataList.count; i++) {
        Mt4Model *model = self.dataList[i];
        model.isCount = YES;
        model.reloadBlock = ^{
            
            if (model.user_type == 1 || model.user_type == 12) {
                //模拟账户
                SimulationCell *cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]];
                 cell.lab1.text = [NSString stringWithFormat:@"%.2f",model.income];
            }else{
                //真实账户
                AccountCell *cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]];
                cell.labe1.text = [NSString stringWithFormat:@"%.2f",model.income];
