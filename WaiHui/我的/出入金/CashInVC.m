    ...
    [BANetManager ba_request_POSTWithEntity:entity successBlock:^(id response) {
        ...
        if ([result[@"code"] intValue] == 1) {
            ...
            if(self.mt4Str.length == 0){
                ...
                self.mt4Str = self.model.mt4_id.firstObject;
...
- (void)loadpassageway:(NSString *)amount
{
    BADataEntity *entity = [BADataEntity new];
    entity.urlString = [NSString stringWithFormat:@"%@%@",MainUrl,Url_MyCenter];
    entity.needCache = NO;
    entity.parameters = @{@"types":@"7",@"Access_Token":[LxUserDefaults objectForKey:Token],@"amount":amount};
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == 0 || indexPath.row == 3) {
        ...
        cell.textField.limitedPrefix = 10;
...
//            [self loadpassageway:dataList[index]];
    ...
    if (indexPath.row == 3) {
        ...
        for (TdModel *model in self.tds) {
            [marray addObject:model.payName];
        }
...
- (IBAction)doneAC:(id)sender {
    ...
    if ([self.outStr isEqualToString:@"暂无可用通道"]) {
        return;
    }
...
//    }
    ...
    [BANetManager ba_request_POSTWithEntity:entity successBlock:^(id response) {
        ...
        if ([result[@"code"] intValue] == 1) {
            ...
            NSURL *url = [NSURL URLWithString:result[@"data"][@"url"]];                             HWBaseWebViewController *vc = [HWBaseWebViewController new];
            vc.url = url;
