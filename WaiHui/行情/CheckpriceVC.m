...
- (IBAction)doneAC:(id)sender {
    ...
    PriceCell *zeroCell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:1 inSection:0]];
    ...
    PriceOneCell *oneCell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:2 inSection:0]];
    if ([oneCell.countView.countTextField.text doubleValue] == 0) {
        [SVProgressHUD showErrorWithStatus:@"请先设置交易手数"];
        return;
    }
    ...
    NSString *commo;
    if ([self.typeStr isEqualToString:@"限价"]){
    ...
    [BANetManager ba_request_POSTWithEntity:entity successBlock:^(id response) {
        ...
        if ([result[@"code"] intValue] == 1) {
            ...
            TransactionResultVC *vc = [TransactionResultVC new];
            vc.model = self.model;
            vc.dic = entity.parameters;
            vc.reloadBlock = ^{
                [self loadData];
            };
            [self.navigationController pushViewController:vc animated:YES];
            [self xw_postNotificationWithName:ListChangeNotice userInfo:nil];
...
- (void)reloadUI
{
    ...
    //止损
    PriceCell *cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:3 inSection:0]];
    ...
    PriceCell *onecell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:1 inSection:0]];

    
    NSString *str = [NSString stringWithFormat:@"%%.%df",self.model.digits];
    onecell.countView.countTextField.limitedSuffix = self.model.digits;
    ...
    if ([self.typeStr isEqualToString:@"限价"]) {
        if (self.type == 0) {
            ...
            NSString *string = [NSString stringWithFormat:@"价格（≤%%.%df）",self.model.digits];
            
            onecell.nameLab.text = [NSString stringWithFormat:string,self.model.bid - pow(10, -self.model.digits) * 50];
            NSString *numStr = [NSString stringWithFormat:str,self.model.bid - pow(10, -self.model.digits) * 50];
            
            onecell.max = numStr;
            onecell.countView.currentCount = numStr;
            ...
            onecell.nameLab.text = [NSString stringWithFormat:string,self.model.ask + pow(10, -self.model.digits) * 50];
...
- (void)countContract
{
    ...
    PriceOneCell *one = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:1 inSection:0]];
...
- (void)handleUI
{
    self.names = @[@"挂单类型",@"限价",@"交易手数",@"止损（<1.1314）",@"止盈（>1.1314）"];
    self.typeStr = @"限价";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == 2) {
        ...
        LotCountCell *cell = [tableView dequeueReusableCellWithIdentifier:identifire];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"LotCountCell" owner:nil options:nil] lastObject];
        ...
        cell.countView.countTextField.limitedSuffix = self.model.lot_step.length - 2;
        cell.countView.digits = self.model.lot_step.length - 2;
        ...
        static NSString *identifire = @"cellZeroID";
        TraniceTypeCell *cell = [tableView dequeueReusableCellWithIdentifier:identifire];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"TraniceTypeCell" owner:nil options:nil] lastObject];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            
        }
        cell.nameLab.text = self.typeStr;
        cell.titleLab.text = @"挂单类型";
        return cell;
...
//    }
    if(indexPath.row == 0){
        TermofvalidityAlert *alert = [[TermofvalidityAlert alloc] initTermofvalidityAlertWith:@"挂单类型" dataList:@[@"限价",@"停损"] seletedStr:self.typeStr];
        alert.clickBlock = ^(NSString * _Nonnull indexStr) {
            if (![self.typeStr isEqualToString:indexStr]) {
                self.typeStr = indexStr;
                [self.tableView reloadData];
            }
